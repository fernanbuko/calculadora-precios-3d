...
- **Escala de tamaño (en milímetros, opcional)**: apagada por defecto para que el peso/tiempo que pongas se refleje tal cual en el resultado. Actívala con el interruptor "¿Cotizar a otro tamaño?" solo cuando quieras eso: mides peso y horas una vez, a una medida de referencia (en mm), y luego cotizas la misma pieza a otra medida cambiando el campo "Nueva medida a cotizar" — el peso y el tiempo se recalculan automáticamente (escalan al cubo del factor lineal entre ambas medidas), sin volver a laminar en el slicer. Al apagar el interruptor, las dos medidas se igualan solas para no dejar una escala vieja aplicada sin que se note.
...
- **Perfiles de impresora/filamento**: la pestaña Perfiles tiene sus propios campos de **modelo**, **precio de la impresora** y **consumo (Watts)** — los llenas ahí directamente al crear el perfil, sin tener que ir primero a la Calculadora a escribirlos, y de paso quedan aplicados en tu cotización actual. El resto (precio/peso de rollo, precio del kWh, vida útil) se guarda con los valores que tengas en ese momento en la Calculadora. Cárgalos con un clic — cada perfil muestra su modelo y su costo por hora, útil para comparar impresoras.
