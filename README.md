...
- **Pedidos**: registra qué le debes entregar a cada cliente y cuánto cobrar. Cada pedido se arma con **artículos** (uno por casilla: nombre, cantidad y precio unitario — ej. "2x Llavero", "1x Muñeco"), con un botón "+ Agregar artículo" para ir sumando más; el total se calcula solo a partir de ahí (no hay que escribirlo aparte). El botón "↑ Agregar precio calculado" añade un artículo con el precio que te dio la calculadora, por si no quieres desglosar. Marca cada pedido como hecho/entregado/pagado con un toque, edítalos cuando cambien los datos, y ve de un vistazo cuánto has generado en total (pedidos ya pagados), cuánto tienes por cobrar y cuántos pedidos faltan.
  ...
  Arriba de todo hay una sección de **"Pendientes"** con tarjetas deslizables (como una app móvil): cada pedido avanza solo por sus tres etapas (hacer → entregar → cobrar) — deslízala hacia la derecha (o toca el botón) y pasa a la siguiente, mostrando el resultado ("Pendiente por entregar", "Pendiente por cobrar"...) mientras deslizas; al completar las tres, desaparece de la cola.
