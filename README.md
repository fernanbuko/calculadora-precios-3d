...
- **Cuentas bancarias y compartir por WhatsApp**: en Finanzas, debajo del resumen de lo generado/por cobrar, el botón "+ Agregar cuenta" crea una tarjeta por cada banco que uses — con su propio nombre (ej. "BCP", "Interbank") y un cuadro de texto libre para los datos (número, alias, titular... lo que necesites). Cada cuenta tiene sus propios botones: "💾 Guardar" (sincroniza a la nube igual que el resto si iniciaste sesión) y "📱 WhatsApp" (abre WhatsApp con exactamente el texto de esa cuenta, sin agregarle nada). Un botón "✕ Eliminar" por cuenta la quita, con confirmación.
